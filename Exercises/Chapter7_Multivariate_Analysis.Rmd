...
Look at teh USV object.  What are its components?
...
How does each pair of singular vectors improve our approximation to Xtwo?
...
Compute the first principal component for the turtles data.
...
What part of the output of the svd function leads us to the first PC coefficients?
...
The screeplot tells us to use a 2 component plot.  These values actually represen the eigenvalues.
...
Numbers appear to go from low to high.
...
Small angles between the vectors Phenols, Flav and Proa indicate they are strongly correlated, whereas Hue and Alcholo are uncorrelated.
...
Yes; refer to sentence above from Wikipedia


"Rank of a matrix is defined as (a) the maximum number of linearly independent column vectors in the matrix or (b) the maximum number of linearly independent row vectors in the matrix. Both definitions are equivalent."
...
Each cell in v is multipled by each cell in v to create a rank-1 matrix (15X4 matrix).
