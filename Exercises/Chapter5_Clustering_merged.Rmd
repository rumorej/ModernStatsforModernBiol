...
It is important to keep in mind that clustering is impacted by the distance algorithm.
...
Function below returns a special object of class dist, and then computes the binary distance
...
Correlation-based distances are commonly used in gene expression analyses to identify clusters of observations with the same overall profiles.  The distance between two objects will be zero if they are perfectly correlated.
...
* PAM Algorithm (Partioning Around Medoids)
  + Select *k* clusters
  + Randomly select *k* objects to become the medoids
  + Assign each observation in the dataset to the nearest medoid to construct clusters
  + Medoids are swapped with non-medoids and distances are recalulated
  + Repeat Step 3-4 until clusters stabilize

* *k-means*
    + Center of a given cluster is calculated as the mean value of all the data points in the cluster
    + How it works...
      + Select *k* clusters
      + Randomly select *k* objects to be the cluster centroids
      + Assign items to the k clusters that contain the closest centroid
      + Re-calculate the centroid for each of the k clusters (i.e., mean value of all the data points in the cluster)
        + Want the centroid to be as small as possible
      + Calculate the distance of all items to the k centroids (i.e., Euclidean distance)
      + Repeat until clusters assignments are stable (or the max # of iterations has been met)
...
Repeating a clustering procedure numerous times on the same data with different starting points can be used to identify tight clusters, which correspond to clusters that are "almost always" grouped together.
...
Of note, each column in the plot generated by the R chunk below corresponds to a cell and the colors represent the cluster assignments.
...
* What is Mass Cytometry?
  ...
  + Uses antibodies labeled with heavy metal isotopes rather than fluorochromes
    + *Note*: an isotope is a variant of an element that differs in atomic mass (i.e., different number of neutrons).
...
Based on the R code chunk below, 41 variables were measured in fcsB, which includes 38 heavy metal isotopes.
...
Based on the functionality of kmeansFilter, which is a one dimensional filter that takes a parameter agruement associated with two or more populations, a single dimension has been used to split the data into two groups.
...
* Clusters correspond to dense regions in the data space, separated by regions of lower density of points
  ...
  + Can handle outliers
  + Limitation
      + Does not do well with clusters of similar density
...
* DBSCAN (Density-based spatial clustering of applications with noise) 
  ...
  * epsilon (“eps”)
    ...
    + If *eps* is too small = large part of the data won't be clustered
    + If *eps* is too large = majority of objects will be in the same cluster (i.e., merging of clusters)
  ...
  * minimum points (“MinPts”)
    ...
    + General Rule = *minpts* >= D + 1, where D corresponds to the number of dimensions
...
+ Agglomerative or Divisive
  + Agglomerative = initally each data point is considered as an individual cluster 
    + Similar clusters merge with other clusters until one cluster or K clusters are formed
...
*Note*: When prior knowledge of cluster size is known (i.e., they are known to be similar in size), average linkage or Ward's method is considered to be the best tactic, however, this is often not something that is known.
...
* Heatmaps can be used to visualize large, matrix-like datasets
  + provides an illustration of the patterns that might be contained within the data.


**Question 5.8**


Use *pheatmap* to generate two simple heatmaps, one for Euclidean and one for Manhattan, without dendrogram or reordering.
...
#Euclidean only
#Generate Matrix
Euc = as.matrix(dist(Morder, method = "euclidean"))

#Create heatmap
pheatmap(Euc, cluster_rows=FALSE, cluster_cols=FALSE, show_colnames = TRUE, show_rownames = TRUE, scale = "column")

#Manhattan only
#Generate Matrix
Man = as.matrix(dist(Morder, method = "manhattan"))

#Create heatmap
pheatmap(Man, cluster_rows=FALSE, cluster_cols=FALSE, show_colnames = TRUE, show_rownames = TRUE)
...
Now include the dendrogram for each.  What differences are noticeable?

```{r}
#Euclidean only
pheatmap(Euc, cluster_rows=TRUE, cluster_cols=TRUE, show_colnames = TRUE, show_rownames = TRUE)

#Manhattan only

pheatmap(Man, cluster_rows=TRUE, cluster_cols=TRUE, show_colnames = TRUE, show_rownames = TRUE)
```

Cells appear to be grouped according to cell type with Euclidean; effector cells are split into two groups when the Manhattan distance is used.


**Question 5.10**


How many ways are there to reorder the tip labels?


There are at least 9 different ways to order the tip labels in Figure 5.22.


*Question*: Is there a type of permutation that can be used to deteremine the true answer?


## Validating and Choosing the # of Clusters

It is important to assess the quality of clustering to ensure the interpretation of the results is well-informed.

A number of different algorithms can be used for cluster validation.


* WSS
  + Within-group sum of squares
  + Maximize the between-groups differences while maintaining a small within-group distance
  + Compare value across different values of *k*
...
To better understand the various methods that can be used to deteremine the number of optimal clusters for the data, it can be helpful to look at cases where the answer is actually known.


**Simulate Data from 4 Groups**


Generate data and combine the four tibbles
...
Re-run the code above and compare the WSS values.


(a) Why are the values different?


The WSS values differ slightly between runs as the process is continually repeated; some datapoints may be assigned to a different cluster, resulting in slightly different results.  However, the optimal number of clusters (i.e., k=4) does not change.
...
k=4 still appears to be the best choice for the number of clusters for simulated uniform data with the same range and dimensions.
...
The largest Gap Statistic is at k=4.
...
Above R code chunk above reports a cluster size of *k* = 9 (i.e., gap is not larger than the first local maximum minus a standard error) and *k* = 7 (choice recommended by Tibshirani et al., 2001).
...
It takes a VERY LONG time to run and causes R Studio to freeze so we will not be running this in class...
...
As an example, let's use the *Hiiragi2013* data to test the following hypothesis:
...
Use the approach by Ohnishi *et al.*, 2014

```{r}
clusterResampling = function(x, ngenes = 50, k = 2, B = 250,
                             prob = 0.67) {
  mat = Biobase::exprs(x) #construct the matrix
  ce = cl_ensemble(list = lapply(seq_len(B), function(b) {
    selSamps = sample(ncol(mat), size = round(prob * ncol(mat)),
                      replace = FALSE) #draw a random subset of the data by selecting 67% of the samples without replacement
    submat = mat[, selSamps, drop = FALSE] #add selSamps to the matrix
    sel = order(rowVars(submat), decreasing = TRUE)[seq_len(ngenes)] #select the top ngene features by overall variance
    submat = submat[sel,, drop = FALSE] #add sel to the matrix
    pamres = pam(t(submat), k = k) #apply k-means clustering but actually looks like k-medoids
    pred = cl_predict(pamres, t(mat[sel, ]), "memberships") #predict cluster memberships of samples that were not in the subset based on proximity to cluster centers
    as.cl_partition(pred) #returns cluster membership obtained from pred
  })) # entire function is repeated B times
  cons = cl_consensus(ce) #apply consensus clustering
  ag = sapply(ce, cl_agreement, y = cons) #measure agreement with consensus clustering
  list(agreements = ag, consensus = cons)
}
```

*Note*: explaination of the R code chunk above was obtained from page 128.
...
Based on the plot created above, 1 indicates perfect agreement and lower values indicate less agreement.  Thus, there is a high agreement that the partion of the mouse blastocyst on embryonic day 3.5 (E3.5) into two clusters is both stable and reproducible.
...
Cluster the two groups according to distance from the group center (i.e., *k*-medoids)
...
* FastQ File
  ...
  + Example
    + Q-Score of 30 indicates a base calling accuracy of 99.9% (1 in 1000 chance the base is wrong)
...
As the base quality increases (i.e., consensus quality score), the error frequency decreases (with the exception of A2A, C2C, G2G, and T2T).
...
```{r}
#Components of dadaRs
length(dadaRs) 
names(dadaRs)

#Components of mergers
length(mergers) 
names(mergers)
```
...
Difficult to get Average Silhouette width close to 1...
...
Hyporadi and Empenigr appear to be highly correlated with respect to location.
...
According the the Figure from the link above, what are some reasons for the clustering and high incidence rates on the West and East Coasts and around Chicago?
